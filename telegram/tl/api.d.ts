...
// namespaces
  ...
  export namespace payments {
    ...
    // MY CLASSES START
    export class GetResaleStarGifts extends VirtualClass<{
      // flags: number is implicit (not passed directly)
      sort_by_price?: boolean;
      sort_by_num?: boolean;
      attributes_hash?: string; // assuming long is string | BigInt
      gift_id: string;
      ...
      static SUBCLASS_OF_ID = 0x7f3b18ea; // обычно это ID, унаследованный от Request
      static classType = "request";
      static className = "payments.getResaleStarGifts";
    
      static fromReader(reader: Reader): GetResaleStarGifts {
        const flags = reader.readInt();
        const attributes_hash = flags & (1 << 0) ? reader.readLong() : undefined;
        const sort_by_price = (flags & (1 << 1)) !== 0;
        const sort_by_num = (flags & (1 << 2)) !== 0;
        const attributes = flags & (1 << 3) ? reader.readVector(t => t.read()) : undefined;
    
        const gift_id = reader.readLong();
        const offset = reader.readString();
        const limit = reader.readInt();
    
        return new GetResaleStarGifts({
          sort_by_price,
          sort_by_num,
          attributes_hash,
          gift_id,
      ...
      getBytes(): Buffer {
        let flags = 0;
        if (this.sort_by_price) flags |= (1 << 1);
        if (this.sort_by_num) flags |= (1 << 2);
        if (this.attributes_hash !== undefined) flags |= (1 << 0);
        if (this.attributes !== undefined) flags |= (1 << 3);
    
        const parts: Buffer[] = [];
        parts.push(Serializer.int(GetResaleStarGifts.CONSTRUCTOR_ID));
        parts.push(Serializer.int(flags));
        if (flags & (1 << 0)) parts.push(Serializer.long(this.attributes_hash!));
        parts.push(Serializer.long(this.gift_id));
        if (flags & (1 << 3)) parts.push(Serializer.vector(this.attributes!, a => a.getBytes()));
        parts.push(Serializer.string(this.offset));
        parts.push(Serializer.int(this.limit));
    
        return Buffer.concat(parts);
      }
    }
    export const GetResaleStarGifts = GetResaleStarGifts;
    //MY CLASSES END
  ...
  export namespace payments {
    ...
    export type TypeGetResaleStarGifts = payments.GetResaleStarGifts;
